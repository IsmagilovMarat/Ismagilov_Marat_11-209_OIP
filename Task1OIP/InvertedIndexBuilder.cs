...
namespace Task1OIP
{
    public class InvertedIndexBuilder
    {
        ...
        public async Task<Dictionary<string, HashSet<int>>> BuildIndexAsync(string docsDirectory)
        {
            ...
            Console.WriteLine($"Найдено {textFiles.Count} файлов для индексации\n");
            ...
            for (int docId = 0; docId < textFiles.Count; docId++)
            {
                ...
                Console.Write($"Индексация [{docId + 1}/{textFiles.Count}] {fileName}... ");
                ...
                try
                {
                    ...
                    // Добавляем в индекс (только уникальные токены для документа)
                    ...
                    foreach (var token in uniqueTokens)
                    {
                        ...
                        invertedIndex[lowerToken].Add(docId + 1); // docId с 1 для читаемости
                    }

                    Console.WriteLine($"✓ {uniqueTokens.Count} уникальных токенов");
                }
                catch (Exception ex)
                {
        ...
        private List<string> Tokenize(string text)
        {
            ...
            // Удаляем HTML-разметку и метаданные Gutenberg
